...
export class ProdshopmodService {
    ...
    getProducts(useCashe = true) : Observable < IProductPagination < IProduct[] >> {
        ...
        if (this.cashe.size > 0 && useCashe) {
            if (this.cashe.has(Object.values(this.shopParams).join('-'))) {
                this.pagination = this
                    .cashe
                    .get(Object.values(this.shopParams).join('-'));
                if (this.pagination) 
        ...
        return this.http.get < IProductPagination < IProduct[] >> (this.url + 'products', {params}).pipe(map(response => {
            this
                .cashe
                .set(Object.values(this.shopParams).join('-'), response);
    ...
    getBrands() {
        ...
        return this.http.get < IBrands[] > (this.url + 'products/brands').pipe(map(brands => this.brands = brands))
    }

    getProductTypes() {
        if (this.types.length > 0) 
            return of(this.types);
        return this.http.get < IProductTypes[] > (this.url + 'products/types').pipe(map(types => this.types = types))
    }
    ...
    saveProductPicture(formData : any) {
        return this
            .http
            .post(this.url + 'products/savePicture', formData);
    }
    ...
    DeleteType(id : number) {
        return this
            .http
            .delete(this.url + 'productType/' + id);
    }

    DeleteBrand(id : number) {
        return this
            .http
            .delete(this.url + 'productBrand/' + id);
    }
